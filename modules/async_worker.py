...
def worker():
    ...
    def handler(async_task, inputted_image_path=None):
        ...
        #input_folder = 'D:/Fooocus/batch input for upscaling'
        uov_method = uov_method.lower()
        #print(f'Input Folder length:')
        #print(len(os.listdir(input_folder)))
        #print(f'UoV_method:')
        #print(uov_method)
        if inputted_image_path is not None:
            uov_input_image = cv2.imread(inputted_image_path)
            uov_input_image = cv2.cvtColor(uov_input_image,cv2.COLOR_BGR2RGB) #fuck this line of code. Who the fuck uses BGR instead of RGB??? Fucking retarded

        if 'upscale (32x)' in uov_method and len(os.listdir(input_folder)) != 0:
            print(f'My shitty for loop is starting!')
            #for filename in os.listdir(input_folder):

                #input_image_path = os.path.join(input_folder, filename)
                #uov_input_image = cv2.imread(input_image_path)
                #print(f'I am upscaling: {input_image_path}')
            ...
            if 'upscale (32x)' in uov_method:
                ...
                for iteration in range(4):
                    ...
                    progressbar(async_task, 13, f'Upscaling image from {str((H, W))} ...lolololololol')
            ...
            print(f'My shitty loop didnt work')
            cn_tasks = {x: [] for x in flags.ip_list}
            for _ in range(4):
                cn_img = args.pop()
                cn_stop = args.pop()
            ...
            if not skip_prompt_processing:
                ...
                for i in range(image_number):
                    ...
                    negative_basic_workloads.append(
                        task_negative_prompt)  # Always use independent workload for negative.
    ...
    while True:
        ...
        if len(async_tasks) > 0:
            ...
            if len(os.listdir(input_folder)) != 0:
                print(f'Upscaling from {input_folder}. If you wish to process images manually, please remove all files from {input_folder}.')
                loop_iteration = 0
                for filename in os.listdir(input_folder):
                    loop_iteration += 1
                    if loop_iteration == len(os.listdir(input_folder)):
                        input_image_path = os.path.join(input_folder, filename)
                        print(f'I am upscaling: {input_image_path}')
                        print(f'Loop iteration: {loop_iteration}')
                        try:
                            handler(task, input_image_path)
                            ...
                            traceback.print_exc()
                            task.yields.append(['finish', task.results])
                    else: #Not sure if this if/else is even necessary. Fucking classes and their shitty reference types. Why don't people use structs
                        taskCopy = copy.deepcopy(task)
                        input_image_path = os.path.join(input_folder, filename)
                        print(f'I am upscaling: {input_image_path}')
                        print(f'Loop iteration: {loop_iteration}')
                        try:
                            handler(taskCopy, input_image_path)
                            build_image_wall(taskCopy)
                            taskCopy.yields.append(['finish', taskCopy.results])
                            pipeline.prepare_text_encoder(async_call=True)
                        except:
                            traceback.print_exc()
                            taskCopy.yields.append(['finish', taskCopy.results])
            else:
                print(f'Upscaling from manual input. If you wish to process images in batch, please add files to {input_folder}.')
                try:
                    handler(task)
                    build_image_wall(task)
                    task.yields.append(['finish', task.results])
